...
export const load = async ({ params }) => {
    ...
    const [appEvent, releaseEvent] = await Promise.all([
        fetchEvent({ kinds: [EVENT_KINDS.APP], authors: [pubkey], '#d': [identifier], ...PLATFORM_FILTER }, { relays: [...DEFAULT_CATALOG_RELAYS], timeout: 10000 }),
        fetchEvent({ kinds: [EVENT_KINDS.RELEASE], '#a': [aTagValue], ...PLATFORM_FILTER, limit: 1 }, { relays: [...DEFAULT_CATALOG_RELAYS], timeout: 10000 })
    ]);
    if (!appEvent) {
        return {
            app: null,
