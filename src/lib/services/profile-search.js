...
function startWatchDefaultProfiles() {
    if (defaultProfilesWatchStarted)
        return;
...
function startWatchUserContacts(userPubkey) {
    if (userWatchStarted.has(userPubkey))
        return;
    userWatchStarted.add(userPubkey);
    // Load kind 3 and 30000 into store (background)
    watchEvents({ kinds: [KIND_CONTACT_LIST], authors: [userPubkey], limit: 1 }, { relays: PROFILE_RELAYS, timeout: 6000 }, (contactEvents) => {
 ...
 */
function profileFromStore(pubkey) {
 ...
 */
function defaultProfileRow(pubkey, isZapstore) {
 ...
 */
function getCandidatePubkeys(userPubkey) {
 ...
 */
function searchFromStore(userPubkey, query) {
    ...
    for (const pubkey of candidates) {
        ...
        if (row)
            withResults.push(row);
    }
    if (normalizedQuery === '') {
        // Empty query: strict default order first (Zapstore + 3 npubs), then rest (max 10)
        const inDefaultOrder = DEFAULT_PUBKEYS.map((pk) => withResults.find((r) => r.pubkey === pk)).filter((r) => !!r);
    ...
    for (const result of withResults) {
        ...
        if (name.includes(normalizedQuery) ||
            ...
            if (displayName.startsWith(normalizedQuery))
                ...
                score += 50;
            if (nip05.startsWith(normalizedQuery))
                score += 80;
            else if (nip05.includes(normalizedQuery))
                score += 30;
            if (pubkeyLower.startsWith(normalizedQuery))
                score += 20;
...
export function createProfileSearch(userPubkey) {
    ...
    return {
        ...
        search: async (query) => searchFromStore(userPubkey, query),
        getProfileCount: () => 0,
...
export function getProfileSearch(userPubkey) {
    if (!userPubkey) {
        return {
            init: async () => { },
            search: async (query) => searchFromStore(null, query),
            getProfileCount: () => 0,
            getPubkeyCount: () => 0,
            isInLists: () => false,
            addProfile: () => { }
