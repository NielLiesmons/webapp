<script lang="ts">
  ...
  function handleClickOutside(event: MouseEvent) {
    ...
    if (catalogDropdownOpen && catalogDropdownContainer && !catalogDropdownContainer.contains(target)) {
      catalogDropdownOpen = false;
    }
  }

  const headerVisible = $derived(
    scrollThreshold == null ? true : scrollY > scrollThreshold
  );
          ...
          {#if catalogDropdownOpen}
            <div class="catalog-dropdown-panel" role="dialog" aria-label="Catalog info">
              <p class="catalog-dropdown-text">
                Zapstore is the default catalog. We're opening Zapstore up to any app catalog in the coming month(s).
              </p>
            </div>
          {/if}
        </div>
  ...
  <div class="floating-menu-bar" role="banner">
    <nav class="floating-menu-nav">
      ...
      >
        ...
        {#if menuOpen}
          <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
          <div class="menu-backdrop" onclick={closeMenu} role="button" tabindex="-1" aria-label="Close menu"></div>
          <div class="menu-dropdown floating-menu-dropdown">
            <div class="menu-header-row">
              <a href="/" class="menu-logo" onclick={closeMenu}>
                <svg width="19" height="32" viewBox="0 0 19 32" fill="none" xmlns="http://www.w3.org/2000/svg" class="menu-logo-icon">
                  <defs>
                    <linearGradient id="menu-logo-gradient-float" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" style="stop-color: hsl(252, 100%, 72%);" />
                      <stop offset="100%" style="stop-color: hsl(241, 100%, 68%);" />
                    </linearGradient>
                  </defs>
                  <path d="M18.8379 13.9711L8.84956 0.356086C8.30464 -0.386684 7.10438 0.128479 7.30103 1.02073L9.04686 8.94232C9.16268 9.46783 8.74887 9.96266 8.19641 9.9593L0.871032 9.91477C0.194934 9.91066 -0.223975 10.6293 0.126748 11.1916L7.69743 23.3297C7.99957 23.8141 7.73264 24.4447 7.16744 24.5816L5.40958 25.0076C4.70199 25.179 4.51727 26.0734 5.10186 26.4974L12.4572 31.8326C12.9554 32.194 13.6711 31.9411 13.8147 31.3529L15.8505 23.0152C16.0137 22.3465 15.3281 21.7801 14.6762 22.0452L13.0661 22.7001C12.5619 22.9052 11.991 22.6092 11.8849 22.0877L10.7521 16.5224C10.6486 16.014 11.038 15.5365 11.5704 15.5188L18.1639 15.2998C18.8529 15.2769 19.2383 14.517 18.8379 13.9711Z" fill="url(#menu-logo-gradient-float)" />
                </svg>
                <span class="menu-logo-text">Zapstore</span>
              </a>
              {#if isConnected}
            ...
            {#if !isConnected}
              <div class="menu-divider"></div>
              <div class="menu-cta-wrapper">
                <button type="button" class="btn-primary w-full" onclick={openGetStartedModal}>Get Started</button>
              </div>
            {/if}
          </div>
        {/if}
