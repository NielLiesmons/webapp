                ...
                if (!database.objectStoreNames.contains('events')) {
                    ...
                    if (oldVersion < 4) {
                        if (!store.indexNames.contains('created_at'))
                            store.createIndex('created_at', 'event.created_at');
                        if (!store.indexNames.contains('pubkey'))
                            store.createIndex('pubkey', 'event.pubkey');
 ...
 */
export async function queryCache(filter) {
    ...
    if (filter.kinds && filter.kinds.length === 1) {
        try {
            records = (await database.getAllFromIndex('events', 'kind', filter.kinds[0]));
        }
        catch {
            // Index may not exist on older DBs — fall back to full scan
            records = (await database.getAll('events'));
        }
    }
    else {
        records = (await database.getAll('events'));
...
function matchesFilter(event, filter) {
    ...
    for (const [key, values] of Object.entries(filter)) {
        if (key.startsWith('#') && Array.isArray(values) && values.length > 0) {
            ...
            const matchingTags = event.tags.filter((t) => t[0] === tagName);
            const hasMatch = matchingTags.some((t) => t[1] && valueSet.has(t[1]));
            if (!hasMatch)
                return false;
        }
    }
 ...
 */
export function queryStore(filter) {
    const store = getEventStore();
    // Start with kind filter via getByFilters (returns NostrEvent[])
    let events = store.getByFilters(filter.kinds ? { kinds: filter.kinds } : {});
    // Filter by authors
    if (filter.authors && filter.authors.length > 0) {
    ...
    for (const [key, values] of Object.entries(filter)) {
        if (key.startsWith('#') && Array.isArray(values) && values.length > 0) {
            ...
            events = events.filter((e) => {
                ...
                const matchingTags = e.tags.filter((t) => t[0] === tagName);
                // Check if any tag value matches
                return matchingTags.some((t) => t[1] && valueSet.has(t[1]));
            });
        }
    }
 ...
 */
export function queryStoreOne(filter) {
 ...
 * );
 */
export async function fetchEvents(filter, options = {}) {
 ...
 */
export async function fetchEvent(filter, options = {}) {
 ...
 * );
 */
export function watchEvents(filter, options = {}, onUpdate) {
 ...
 */
export function watchEvent(filter, options = {}, onUpdate) {
    const results = watchEvents({ ...filter, limit: 1 }, options, onUpdate ? (events) => onUpdate(events[0] ?? null) : undefined);
    return results[0] ?? null;
}
/**
 ...
 */
export function hasCachedEvents(kind) {
    const store = getEventStore();
 ...
 */
export async function fetchFromRelays(relays, filter, options = {}) {
 ...
 */
function parseAppReference(event) {
 ...
 */
export async function fetchAppsByReleases(relays, limit = 20, until, options = {}) {
    const { timeout = 5000, signal } = options;
    // Check if already aborted
    if (signal?.aborted) {
 ...
 */
export async function fetchAppStacks(relays, limit = 20, until, options = {}) {
    const { timeout = 5000, signal } = options;
    if (signal?.aborted) {
 ...
 */
export async function fetchApp(pubkey, identifier, options = {}) {
    const { timeout = 5000, signal } = options;
    if (signal?.aborted) {
        return null;
    }
...
export async function fetchAppStacksParsed(options = {}) {
    ...
    const stacks = events.map((event) => parseAppStack(event));
    // Sort by created_at descending
 ...
 */
function buildCommentFilter(pubkey, identifier, aTagKind = 32267) {
 ...
 */
export function queryCommentsFromStore(pubkey, identifier, aTagKind = 32267) {
 ...
 */
export function watchComments(pubkey, identifier, options = {}, onUpdate) {
    if (!pubkey || !identifier)
        return [];
 ...
 */
function buildZapFilter(pubkey, identifier) {
...
export async function fetchZaps(pubkey, identifier, options = {}) {
    ...
    const mainResults = await Promise.all(filtersMain.map((f) => fetchEvents(f, { relays, timeout, signal })));
 ...
 */
export function watchZaps(pubkey, identifier, options = {}, onUpdate) {
...
export function parseZapReceipt(event) {
    ...
    if (receiptETag?.[1])
        result.zappedEventId = receiptETag[1].toLowerCase();
    ...
    if (descTag && descTag[1]) {
        try {
            ...
            if (result.zappedEventId == null) {
                ...
                if (eTag?.[1])
                    result.zappedEventId = eTag[1].toLowerCase();
            }
...
export async function publishComment(content, target, signEvent, emojiTags, parentEventId, replyToPubkey, parentKind) {
    ...
    if (parentEventId) {
        ...
        if (!/^[a-f0-9]{64}$/.test(eId)) {
            throw new Error(`Invalid parent event id for comment (expected 64 hex chars): ${parentEventId.slice(0, 20)}...`);
        }
        tags.push(['e', eId]);
